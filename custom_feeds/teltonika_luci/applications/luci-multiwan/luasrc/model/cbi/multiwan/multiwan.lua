...
local moduleproto = luci.util.trim(luci.sys.exec("uci get network.ppp.proto"))
...
m = Map("multiwan", translate("Backup WAN"))
m:chain("network")
m:chain("firewall")
...
s = m:section(NamedSection, "config", "multiwan", translate("Backup Link"))

if wanifname == moduleIfname or wanifname == "eth2" or wanifname == "wm0" then
	if wanifname == moduleIfname then
		e = s:option(Flag, "enabled", translate("Enable"), translate("Mobile selected as WAN (Wide Area Network) - cannot enable backup link"))
	else
		e = s:option(Flag, "enabled", translate("Enable"), translate("WiMAX selected as WAN (Wide Area Network) - cannot enable backup link"))
	end
	e.hardDisabled = true
else
	local hasssid = wifi_sta_static()
	local bridge_on = luci.util.trim(luci.sys.exec("uci get network.ppp.bridge"))
	if hasssid and wanifname == "wlan0" then
		e = s:option(Flag, "enabled", translate("Enable"), translate("Wireless SSID (Service Set Identifier) is not set - cannot enable backup link"))
		e.hardDisabled = true
	elseif bridge_on == "1" then
		e = s:option(Flag, "enabled", translate("Enable"), translate("Cannot enable backup link in bridge mode"))
		e.hardDisabled = true
	else
		e = s:option(Flag, "enabled", translate("Enable"), translate("Here you can setup your backup WAN (Wide Area Network). If your conventional WAN connection, such as wired Ethernet or Wireless, fails, the backup link will be enabled and take over to keep the router connected"))
	end
end
...
function e.write(self, section, value)
	...
	if CurrentState ~= PreviousState then
		if CurrentState == "1" and (wanifname == moduleIfname or wanifname == "eth2" or wanifname == "wm0") then
			if wanifname == moduleIfname then
				m.message = translate("wrn:%s selected as WAN - cannot enable backup link.", modulservice)
			else
...
s = m:section(NamedSection, "wan", "interface", translate("Backup Configuration"),
	translate("If mobile is selected as WAN, you cannot enable backup link. \nTiming and other parameters will indicate how and when it will be determined that your conventional connection has gone down."))
s.addremove = false
...
fail = s:option(ListValue, "health_fail_retries", translate("Attempts before WAN failover"), translate("Failed ping attempts\\' count before switching to backup WAN (Wide Area Network)"))
fail:value("1", "1")
fail:value("3", "3")
fail:value("5", "5")
fail:value("10", "10")
...
s = m:section(NamedSection, "wan2", "interface", translate("Backup Check"),
	translate("A remote host that will be used to test whether your backup link is alive."))
s.addremove = false

o = s:option(Value, "icmp_hosts", translate("ICMP host"), translate("A remote host to test backup link"))
o.optional = false
o.datatype = "ip4addr"
