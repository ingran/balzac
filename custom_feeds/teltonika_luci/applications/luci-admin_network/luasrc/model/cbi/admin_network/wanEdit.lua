...
if interfaceName ~= "3g-ppp" and interfaceName ~= "none" and interfaceName ~= "wwan0" then
...
function m.on_commit()
	--if interfaceName == "3g-ppp" or interfaceName == "eth2" or interfaceName == "wwan0" then
	--	m.uci:delete("network", __define__network, "dns")
...
ms = m2:section(NamedSection, __define__network, "interface", translate("Backup Configuration"),
...
fail = ms:option(ListValue, "health_fail_retries", translate("Attempts before failover"), translate("Failed ping attempts\\' count before switching to backup connection"))
	fail:value("1", "1")
	fail:value("3", "3")
	fail:value("5", "5")
	fail:value("10", "10")
