...
s2 = m:section(NamedSection, "cfgsms", "cfgsms", translate("Configuration Message"))
...
s3 = m:section(NamedSection, "message", "message", translate("Send Configuration Message"))
...
function o.write(self, section, value)
	...
	local tel_num = luci.http.formvalue("cbid."..self.config.."."..section..".tel")
	...
	if sendPressed then
		if value and value ~= "" and tel_num then
			local tmpFile = "/tmp/conf_msg"
			local tmpStatus = "/tmp/sms_status"
			write_to_file(tmpFile, value)
			--os.execute("/sbin/sms_utils/send_conf_messages request & 2>/dev/null >/dev/null")
			if authorisation and authorisation ~= "" then
				--os.execute("echo '" ..string.format('/usr/sbin/gsmctl -S -s "%s %s %s"', tel_num, authorisation, command).."'>/tmp/lua.test")
				output = assert (io.popen(string.format('/usr/sbin/gsmctl -S -s "%s %s get_configure"', tel_num, authorisation)))
			else
				output = assert (io.popen(string.format('/usr/sbin/gsmctl -S -s "%s get_configure"', tel_num)))
			end
			local t = output:read()
			...
			if t == "OK" then
				...
				m.uci:set("sms_utils", "message", "number", tel_num)
				m.uci:commit("sms_utils")
			elseif t then
...
function o.cfgvalue(self, section)
	local generatePressed = luci.http.formvalue("cbid.sms_utils.cfgsms._btn")
	...
	if generatePressed then
		...
		--count_messages(commands..", " ..vpn_commands)
		if commands ~= "" and vpn_commands ~= "" then
			return commands ..", " ..vpn_commands
