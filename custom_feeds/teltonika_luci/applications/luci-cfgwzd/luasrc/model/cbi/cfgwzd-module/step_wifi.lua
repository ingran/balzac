...
function m.on_after_save()
	if m:formvalue("cbi.wizard.finish") then
		x = uci.cursor()
		networkIface = x:get( "system", "module", "iface" )
		x:set( "network", "wan", "ifname", networkIface or "eth1" )
		x:commit("network")
...
ssid.rmempty = false
ssid.datatype = "lengthvalidation(0,64)"
...
ch = s:option(Value, "channel", translate("Channel"), translate("Your wireless radio is forced to work in selected channel in order to maintain the connection"))
