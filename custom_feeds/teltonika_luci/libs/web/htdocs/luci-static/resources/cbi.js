...
function lan_new_url(url){
	var e = document.getElementById("cbid.network.lan.ipaddr");
	if (e){
		e = e.value;
		...
		if (full_link.search("https://") != -1) {
			sesion = "https://"
		}

		var ip_addr = full_link.split(sesion)[1];
		ip_addr = ip_addr.split("/")[0];
		if (port && port != "")
			var link = sesion + e + ":" + port + url;
		else
			var link = sesion + e + url;
		var e_ipaddr = e + ":" + port;

		if (ip_addr != e_ipaddr){
			setTimeout(redirect, 10000, link);
		}
	}
}
...
function redirect(url, input){
	if (input == null){
		...
		var check_field = document.getElementById("focus_password")
		setTimeout(redirect, 5000, url, check_field);
	}
}

function mode_list_check(config, section, option1, option2){
...
function cfgwzd_lan(url){
	var ipaddr=document.getElementById("cbid.network.lan.ipaddr").value;
	var netmask=document.getElementById("cbid.network.lan.netmask").value;
	var igs=document.getElementById("cbid.network.lan.igs").checked;
	var limit=document.getElementById("cbid.network.lan.limit").value;
	var start=document.getElementById("cbid.network.lan.start").value;
	var leasetime=document.getElementById("cbid.network.lan.leasetime").value;
	var vfalse=0;
	if(!ValidateIPaddress(ipaddr)){
		document.getElementById("cbid.network.lan.ipaddr").className = "cbi-input-text cbi-input-invalid";
		vfalse = 1;
	}else
		document.getElementById("cbid.network.lan.ipaddr").className = "cbi-input-text";

	if(!ValidateIPaddress(netmask)){
		document.getElementById("cbid.network.lan.netmask").className = "cbi-input-text cbi-input-invalid";
		vfalse = 1;
	}else
		document.getElementById("cbid.network.lan.netmask").className = "cbi-input-text";
	if(vfalse == 1)
		return false
	var div = document.createElement('spin-loader');
	...
	XHR.get(url, { sesion: link, ipaddr: ipaddr, netmask:netmask, igs:igs, limit:limit, start:start, leasetime:leasetime },
		function(x, info){
			if (info){
				if(info.new_ip==1){
					var i =0;
					var interval = window.setInterval(function() {
						...
						img.onload = function() {
							...
							window.location.href = info.new_link;
						};
						if(i>12){
							window.clearInterval(interval);
							window.location.href = info.new_link;
						}
						img.src = info.img_link + Math.random();
						i++;
					}, 5000);
				}else{
					div.style="display: none;";
