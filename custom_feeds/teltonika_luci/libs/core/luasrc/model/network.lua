...
function protocol._ubus(self, field)
	-- workaround
	local prot = utl.trim(sys.exec("uci get network.ppp.proto"))
...
function get_openvpn()
	tablenew = {}
	returnTable = {}
	local opevpns = utl.trim(sys.exec("uci show openvpn | grep =openvpn | grep -E '(openvpn.server_|openvpn.client_)' | awk -F '.' '{print $2 }' | awk -F '=' '{print $1 }'"))

	for openvpn_fullname in opevpns:gmatch("[^\r\n]+") do
		opevpns_status = "Not connected"
		openvpn_name = ""
		openvpn_type = ""
		opevpns_dev = ""
		opevpns_log = ""
		...
		openvpn_type, openvpn_name = openvpn_fullname:match("([^,]+)_([^,]+)")
		opevpns_enable = utl.trim(sys.exec("uci get openvpn."..openvpn_fullname..".enable 2>/dev/null"))
		opevpns_dev = utl.trim(sys.exec("uci get openvpn."..openvpn_fullname..".dev 2>/dev/null"))
		opevpns_log = utl.trim(sys.exec("uci get openvpn."..openvpn_fullname..".status 2>/dev/null"))
		ifinterface = utl.trim(sys.exec("ifconfig "..opevpns_dev.." 2>/dev/null >/dev/null; echo $?"))
		opevpns_status = opevpns_dev
		if opevpns_dev == "tap" then
			opevpns_status = "Bridge"
			...
			opevpns_time = tonumber(now) - tonumber(started)
		elseif ifinterface == "0" or ifinterface == 0 then
			opevpns_status = "Connected"
			opevpns_ip = utl.trim(sys.exec("ifconfig "..opevpns_dev.." 2>/dev/null | grep 'inet addr:' | awk -F':' '{print $2}' | awk -F' ' '{print $1}'"))
			opevpns_mask = utl.trim(sys.exec("ifconfig "..opevpns_dev.." 2>/dev/null | grep 'Mask:' | awk -F':' '{print $4}'"))

			started = utl.trim(sys.exec("cat /tmp/state/ifuptime.log 2>/dev/null | grep -w "..opevpns_dev.." | awk -F'=' '{print $2}' "))
			if not tonumber(started) then
				started=0;
			end
			now = utl.trim(sys.exec("cat /proc/uptime | awk -F'.' '{print $1}'"))
		...
		if openvpn_type == "server" then
			...
			local com = 0
			tableconsinc = {}
			local file = utl.trim(sys.exec("cat "..opevpns_log.." 2>/dev/null"))
...
function get_services()
	...
	table.insert(returnTable, tablenew)
	---------------------OpenVPN[1]---------------------------------
	local openvpn_server = utl.trim(sys.exec("uci show openvpn | grep openvpn.server_ | grep -w enable=1 | wc -l")) or "0"
	local openvpn_client = utl.trim(sys.exec("uci show openvpn | grep openvpn.client_ | grep -w enable=1 | wc -l")) or "0"
...
function new_get_info()
	...
	local data_want = {"netstate", "state", "signal","oper", "ntype", "imei", "imsi", "iccid", "txbytes", "rxbytes", "simstate", "pinstate",
					   "cellid", "rscp", "ecio", "rsrp", "rsrq", "sinr", "pinleft"}
	local returnTable = {}
	...
	if bridge_on then
		intf = utl.trim(sys.exec("uci get network.ppp.ifname"))
	else
		intf = utl.trim(sys.exec("uci get system.module.iface"))
	end
	local options = {
		["1BC7:0021"] = {command="gsmctl -NgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "},--Telit
		["1BC7:1201"] = {command="gsmctl -VgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "},--Telit LTE
		["1BC7:0036"] = {command="gsmctl -VgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "},--Telit LTE
		["12D1:1573"] = {command="gsmctl -UgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "}, --huawei
		["12D1:15C1"] = {command="gsmctl -UgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "}, --huawei TD LTE
		["12D1:15C3"] = {command="gsmctl -UgjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "}, --huawei ME906s LTE
		["1199:68C0"] = {command="gsmctl -gjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "}, --Siera
		["05C6:9215"] = {command="gsmctl -gjqotixJ -e "..intf.." -r "..intf.." -zu -C -X -E -W -M -Z -Bn "} --Quictel
	}
