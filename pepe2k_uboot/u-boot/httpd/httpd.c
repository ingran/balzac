	...
	if(start){
		...
		if(end){
			...
			if(end){
				...
				if(end){
					...
					end = (char *)strstr((char *)start, "name=\"database\"");

					if(end){
						printf("Upgrade type: delete database\n");
						webfailsafe_upgrade_type = WEBFAILSAFE_UPGRADE_TYPE_DATABASE;
		...
		if(end){
			...
			if((end - (char *)uip_appdata) < uip_len){
				...
				// U-Boot
				if((webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT) && (hs->upload_total != WEBFAILSAFE_UPLOAD_UBOOT_SIZE_IN_BYTES+11)){

					printf("## Error: wrong file size, should be: %d bytes!\n", WEBFAILSAFE_UPLOAD_UBOOT_SIZE_IN_BYTES+11);
					webfailsafe_upload_failed = 1;
				...
				if(hs->state == STATE_UPLOAD_REQUEST){
					...
					if(hs->upload >= hs->upload_total){
						if(webfailsafe_upgrade_type == WEBFAILSAFE_UPGRADE_TYPE_UBOOT){
							crc_string[0] = 0;
							ptr = (char *)WEBFAILSAFE_UPLOAD_RAM_ADDRESS;
							strncpy(crc_string, (const char *)(ptr + WEBFAILSAFE_UPLOAD_UBOOT_SIZE_IN_BYTES), 10);
							crc_string[11] = 0;

							//printf("\nout: %s\n\n", crc_string);

							crc = atoi(crc_string);
							crc_current = crc32(0, (const unsigned char *)WEBFAILSAFE_UPLOAD_RAM_ADDRESS, WEBFAILSAFE_UPLOAD_UBOOT_SIZE_IN_BYTES);
