...
function update()
	...
	if results and db then
		...
		if not device_associated(results.mac) then
...
function device_associated(mac)
	local t
	
	if mac then
		mac = mac:gsub("-", ":")
		local h = io.popen(string.format("iwinfo wlan0 assoclist | grep %s", mac))
		t = h:read()
		h:close()
	end
...
if #arg > 0 and #arg <= 5  then
	...
	elseif arg[1] == "assoc" then if device_associated(arg[2]) then print("associated") else print("disassociated") end
	else help() end
else
	help()
end