...
generate_collectibles() {
	...
	for cmd in connstate netstate imei model manuf serial revision imsi simstate pinstate signal operator opernum conntype temp
		do
...
generate_root() {
	...
	# export /etc
	mkdir $ROOT_DIR/etc
	cp -r /etc/config /etc/crontabs /etc/dropbear /etc/firewall.user /etc/group /etc/hosts /etc/inittab /etc/passwd /etc/profile /etc/rc.local /etc/shells /etc/sysctl.conf /etc/uhttpd.crt /etc/uhttpd.key $ROOT_DIR/etc
	# export /tmp
	mkdir $ROOT_DIR/tmp
